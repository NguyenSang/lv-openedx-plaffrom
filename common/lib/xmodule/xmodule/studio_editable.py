...
class StudioEditableBlock(object):
    ...
    def render_children(self, context, fragment, can_reorder=False, can_add=False):
        ...
        """
        contents = []
        storetest = modulestore()
        #results = modulestore().get_courses()
        #xblocktest = storetest.get_items(context["key"])
        #for data in xblocktest:
            #for child in data.get_children():  # pylint: disable=no-member
        for child in self.get_children():  # pylint: disable=no-member
                if can_reorder:
                    context['reorderable_items'].add(child.location)
                context['can_add'] = can_add
                #child.update({"has_author_view": True})
